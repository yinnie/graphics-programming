class TriContainer extends NodeGeom {
...
float         subdivLength;
  ...
  void createMesh() {
    ...
    for (int y = 0; y < subdivisionsY + 1; y++) {
      for(int x = 0; x < subdivisionsX + 1; x++) { 
        addVertex( new PVector( startX + x * subdivLength, startY + y * subdivLength, 0.0 ) );
      }
    }
    ...
    for (int y = 0; y < subdivisionsY; y++) {
      for(int x = 0; x < subdivisionsX; x++) {  
        ...
        int iA = (y)     * (subdivisionsX + 1) + (x);
        int iB = (y)     * (subdivisionsX + 1) + (x + 1);
        int iC = (y + 1) * (subdivisionsX + 1) + (x + 1);
        int iD = (y + 1) * (subdivisionsX + 1) + (x);
  ...
  void setSubdivLength(float isubdivlength) {
    subdivLength = isubdivlength;
  }
