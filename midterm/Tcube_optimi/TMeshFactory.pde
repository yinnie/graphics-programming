...
class TMeshFactory {
  ...
  TCube createCube ( int dimU, float radius) {
    ...
    TCube cube = new TCube(mGlobals);
    
    for (int i = 0; i < 6; i++) {
      cube.cubemeshes[i] = initMesh(dimU, dimU);
    }
    ...
    for (int i = 0; i<6; i++) {
      int vCount = cube.cubemeshes[i].mVertices.size();
      if (i == 0) {  //front
        for (int j = 0; j < vCount; j++) {
          PVector currUV = cube.cubemeshes[i].mVertices.get(j).getUV();
          cube.cubemeshes[i].mVertices.get(j).setPosition( new PVector( currUV.x * radius*2 - halfLen, currUV.y * radius*2 - halfLen, -halfLen ) );
        }
      }
      if(i==1) {     //back
            for (int j = 0; j < vCount; j++) {
      ...
      if(i==2) {     //right
            for (int j = 0; j < vCount; j++) {
          PVector currUV = cube.cubemeshes[i].mVertices.get(j).getUV();
          cube.cubemeshes[i].mVertices.get(j).setPosition( new PVector( currUV.y * radius*2 - halfLen, -halfLen, -currUV.x * radius*2 + halfLen ) );
        }    
      }
           if(i==3) {     //left
            for (int j = 0; j < vCount; j++) {
          ...
          cube.cubemeshes[i].mVertices.get(j).setPosition( new PVector( currUV.y * radius*2 - halfLen, halfLen, -currUV.x * radius*2 +halfLen ) );
           ...
           if(i==4) {     //left
            for (int j = 0; j < vCount; j++) {
          PVector currUV = cube.cubemeshes[i].mVertices.get(j).getUV();
          cube.cubemeshes[i].mVertices.get(j).setPosition( new PVector( -halfLen, currUV.y * radius*2 - halfLen,  currUV.x * radius*2 - halfLen ) );
        }    
      }
           if(i==5) {     //right
            for (int j = 0; j < vCount; j++) {
    ...
    for ( int i = 0; i<6; i++) {
     // Compute bounding box for mesh geometry
    cube.cubemeshes[i].computeBounds();
    // Initialize accelerated model
    initModelAcceleration( cube.cubemeshes[i] );
    }
  ...
  void initModelAcceleration(TMesh iMesh) {
    ...
    iMesh.mModel.beginUpdateColors();
  for (int i = 0; i < tVertCount; i++) iMesh.mModel.updateColor(i, 255, 0, 0, 50);
  iMesh.mModel.endUpdateColors(); 
  
  // Setting model shininess.
  iMesh.mModel.setShininess(16);
    //iMesh.mModel.setColors(255, 0, 0);
